...
public class 龙炎升 : ISlotResolver
{
    public int Check()
    {
        ...
        if (!DRGHelper.龙炎升预备) return (int)CheckResult.状态不符;

        if (CooldownHelper.GetCooldownRemaining(DRGHelper.龙炎升) > 0) return (int)CheckResult.冷却中;
    ...
    {
        slot.Add(new Spell { Id = DRGHelper.龙炎升, TargetType = SpellTargetType.Target, Type = SpellType.Ability });
    }
}
