...
public class 续剑 : ISlotResolver
{
    ...
    {
        uint spellId;
        if (GameHelper.HasStatus(1842))
            spellId = GNBHelper.撕喉;
        else if (GameHelper.HasStatus(1843))
            spellId = GNBHelper.裂膛;
        else if (GameHelper.HasStatus(1844))
            spellId = GNBHelper.穿目;
        else if (GameHelper.HasStatus(3839))
            spellId = GNBHelper.命运之印;
        else if (GameHelper.HasStatus(2686))
