...
public class 暗影锋AOE : ISlotResolver
{
    public int Check()
    {
        if (HelperRuntime.GetCurrentLevel() < 74) return (int)CheckResult.等级不足;
        ...
        if (CooldownHelper.GetCooldownRemaining(DRKHelper.暗黑波动) > 0) return (int)CheckResult.冷却中;
    ...
    {
        slot.Add(new Spell { Id = DRKHelper.暗黑波动, TargetType = SpellTargetType.Target, Type = SpellType.Ability });
    }
}
